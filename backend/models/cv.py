...
class PersonalInfo(EntityMeta):
    ...
    cv: Mapped[uuid.UUID] = mapped_column(ForeignKey('cv.id'), nullable=False)
...
class Skill(EntityMeta):
    __tablename__ = "skill"
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    competence: Mapped[uuid.UUID] = mapped_column(ForeignKey('competence_profile.id'), nullable=False)
...
class Responsibility(EntityMeta):
    ...
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    profile: Mapped[uuid.UUID] = mapped_column(ForeignKey('career_profile.id'), nullable=False)
    name: Mapped[str] = mapped_column(nullable=False)
...
class CV(EntityMeta):
    __tablename__ = 'cv'
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    personal_info: Mapped[PersonalInfo] = relationship()
